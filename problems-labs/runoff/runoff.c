...
bool vote(int voter, int rank, string name)
{
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            preferences[voter][rank] = i; //this will store the rank given in function argument to the voted candidate
            return true;
        }
    }
    return false;
...
bool is_tie(int min)
{
    ...
    for (int i = 0; i < voter_count; i ++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            if (preferences[i][j] != preferences[i + 1][j] && !candidates[j].eliminated)
            {
                return false;
            }
        }
